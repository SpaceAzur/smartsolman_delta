...
class Normalisation(object):
    ...
    RATIO = 0.92
    LEMMA_FR = json.load(open("/data/frLemmaBase_unicode_val_in_keys.json"))
    ...
    def getUniqueWords(self, corpus_normalized: list, numbers: list):
        ...
        with open("/data/dev/SparseMatrixSimilarity/DELTA/full_model_buffer/full_dico",'w') as f:      # je sauvegarde le dictionnaire du corpus
        ...
        with open("/data/dev/SparseMatrixSimilarity/DELTA/full_model_buffer/full_dico_unique",'w') as f:     # je sauvegarde le dictionnaire de mot unique du corpus
            json.dump(unique,f)

        # sauvegarde en CSV
        with open("/data/dev/SparseMatrixSimilarity/DELTA/full_model_buffer/dict_unique.csv", "w") as csv_file:  
