...
# détermine le seuil au-dessus duquel un mot est considéré comme trop frequent (type: integer, ex: 12 = 12%)
SEUIL_FREQUENCE = 12
...
with open('../../../src/base_de_donnees/full/BOW_10092020.json', encoding='utf8') as f:
    data = json.load(f)
...
for texte in doc:
    ...
    for token in texte:
        ...
        frequency[token] = round(frequency[token], 6)   # on arrondi l'indice de frequence 
...
texts = [[token for token in texte if frequency[token] < SEUIL_FREQUENCE if not token in stopwords_fr] for texte in doc]
...
tfidf_custom = models.TfidfModel(corpus=corpus, dictionary=dictionary, smartirs=param, slope=0.2)      # VERSION CUSTOM
print("pivot", tfidf_custom.pivot)
print("slope", tfidf_custom.slope)
